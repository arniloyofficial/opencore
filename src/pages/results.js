...
import { SectionIcon, BandIcon } from '../components/SectionIcons';
...
function RadarChart({ sectionScores }) {
  ...
  function point(i, fraction) {
    const a = angle(i);
    return { x: cx + maxR * fraction * Math.cos(a), y: cy + maxR * fraction * Math.sin(a) };
  }
      ...
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ overflow: 'visible', maxWidth: '100%' }}>
        <defs>
          {sectionScores.map((_, i) => (
            <filter key={i} id={`radar-glow-${i}`} x="-50%" y="-50%" width="200%" height="200%">
        ...
        {sectionScores.map((sec, i) => {
          ...
          return (
            <g key={i}>
              ...
              {isHovered && <circle cx={p.x} cy={p.y} r={10} fill={`${sec.color}25`} stroke={`${sec.color}60`} strokeWidth="1" />}
              <circle cx={p.x} cy={p.y} r={isHovered ? 6 : 4} fill={sec.color} stroke={`${sec.color}80`} strokeWidth="2"
...
export default function Results() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(resultCardRef.current, { backgroundColor: '#0d0d14', scale: 2, useCORS: true, logging: false });
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) { try { await navigator.share({ title: 'My OpenCore Results', text }); } catch (e) {} }
    else { await navigator.clipboard.writeText(text); alert('Result text copied to clipboard!'); }
  };
  ...
  const shareOnFacebook = () => {
    window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent('https://opencore-app.vercel.app')}`, '_blank');
  };

  return (
    <>
      <Head><title>OpenCore — Your Results</title></Head>
        ...
        <div className={styles.container}>
          ...
          <div className={styles.actionsBar}>
            ...
            <div className={styles.shareGroup}>
              <button className={styles.actionBtn} onClick={shareOnTwitter} title="Share on X/Twitter"><XIcon /></button>
              <button className={styles.actionBtn} onClick={shareOnWhatsApp} title="Share on WhatsApp"><WhatsAppIcon /></button>
              <button className={styles.actionBtn} onClick={shareOnFacebook} title="Share on Facebook"><FacebookIcon /></button>
              <button className={styles.actionBtn} onClick={handleShare} title="Share / Copy"><ShareIcon /></button>
              <button className={`${styles.downloadBtn} ${downloading ? styles.btnLoading : ''}`} onClick={handleDownload} disabled={downloading}>
                <DownloadIcon /> {downloading ? 'Saving…' : 'Save as JPG'}
              </button>
            </div>
          </div>
          ...
          <div ref={resultCardRef} className={styles.resultCard}>
            <div className={styles.cardHeader}>
              <div className={styles.logoSmall}><span>⬡</span> OpenCore</div>
            ...
            <div className={styles.scoreHero}>
              <div className={styles.scoreRing} style={{ '--band-color': band.color }}>
                ...
                <svg className={styles.ringProgress} viewBox="0 0 120 120">
                  ...
                  <circle cx="60" cy="60" r="54" fill="none" stroke={band.color} strokeWidth="6" strokeLinecap="round"
              ...
              <div className={styles.bandInfo}>
                {/* BandIcon instead of band.icon emoji */}
                <div className={styles.bandIcon}>
                  <BandIcon id={band.icon} size={28} color={band.color} />
                </div>
              ...
              {sectionScores.map((s, i) => (
                <div key={s.id} className={styles.sectionRow} style={{ animationDelay: `${i * 60}ms` }}>
                  <div className={styles.sectionMeta}>
                    {/* SectionIcon instead of s.emoji */}
                    <span className={styles.sectionEmoji}>
                      <SectionIcon id={s.icon} size={14} color={s.color} />
                    </span>
                    <span className={styles.sectionName}>{s.title}</span>
                    <span className={styles.sectionScore}>{s.score}<span>/{s.maxScore}</span></span>
                  </div>
                  <div className={styles.barTrack}>
                    <div className={styles.barFill} style={{ width: `${s.pct}%`, background: s.color, animationDelay: `${300 + i * 80}ms` }} />
                  </div>
...
function RefreshIcon() {
  return <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10" /><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10" /></svg>;
}
function ShareIcon() {
...
function XIcon() {
  return <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.748l7.73-8.835L1.254 2.25H8.08l4.26 5.633 5.905-5.633zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>;
}
...
function ExternalLinkIcon() {
  return <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, opacity: 0.4 }}><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /></svg>;
}