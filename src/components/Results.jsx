...
function RadarChart({ sectionScores }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", marginBottom:"2rem" }}>
      <div style={{ fontSize:"0.72rem", color:"#475569", textTransform:"uppercase", letterSpacing:"0.12em", marginBottom:"1rem" }}>Dimension Map</div>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ overflow:"visible", maxWidth:"100%" }}>
        <defs>{sectionScores.map((_, i) => (<filter key={`glow-${i}`} id={`glow-${i}`} x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="3" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>))}</defs>
        ...
        {sectionScores.map((sec, i) => { const p = point(i, dataFractions[i]); const isHovered = hoveredIdx === i; return (<g key={i}><title>{sec.title}: {sec.pct}%</title>{isHovered && (<circle cx={p.x} cy={p.y} r={10} fill={`${sec.color}25`} stroke={`${sec.color}60`} strokeWidth="1" />)}<circle cx={p.x} cy={p.y} r={isHovered ? 6 : 4} fill={sec.color} stroke={`${sec.color}80`} strokeWidth="2" filter={`url(#glow-${i})`} style={{ cursor:"default", transition:"r 0.15s ease" }} onMouseEnter={() => setHoveredIdx(i)} onMouseLeave={() => setHoveredIdx(null)} />{isHovered && (<text x={p.x} y={p.y - 12} textAnchor="middle" fontSize="10" fontWeight="700" fill={sec.color} style={{ pointerEvents:"none", fontFamily:"'DM Sans', sans-serif" }}>{sec.pct}%</text>)}</g>); })}
      </svg>
    </div>
  );
}
...
export default function Results({ answers, onRetake }) {
  ...
  // Save to Supabase + localStorage once on mount
  useEffect(() => {
    ...
    saveToHistory({
      ...
      sectionScores: sectionScores.map(s => ({ id: s.id, title: s.title, icon: s.icon, color: s.color, pct: s.pct })),
  ...
  async function saveAsJpg() {
    ...
    try {
      ...
      const canvas = await html2canvas(poster, { backgroundColor: "#0a0010", scale: 2, useCORS: true, logging: false, width: 405, height: 720 });
  ...
  async function copyLink() {
    const url = token ? `https://tryopencore.vercel.app/r/${token}` : `https://tryopencore.vercel.app`;
    try {
  ...
  function getShareUrl() { return token ? `https://tryopencore.vercel.app/r/${token}` : `https://tryopencore.vercel.app`; }
  function getShareText() { return `I just completed the OpenCore Mental Health Assessment.\nResult: ${band.label} (${totalScore}/${MAX_SCORE})\nSee my results: ${getShareUrl()}`; }
  ...
  function shareOnFacebook() { window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(getShareUrl())}`, "_blank"); }

  const dateStr = new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" });
  const btnBase = { border:"none", borderRadius:12, padding:"0.85rem 1.5rem", fontSize:"0.88rem", fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:"0.5rem", fontFamily:"'Syne', sans-serif", transition:"all 0.2s ease" };
  const iconBtnBase = { border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, padding:"0.85rem 1.1rem", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(255,255,255,0.05)", transition:"all 0.2s ease" };
      ...
      <div style={{ maxWidth:720, margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"2rem", paddingBottom:"1.5rem", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:"0.6rem" }}>
            <img src="/logo.svg" alt="OpenCore" style={{ height:26 }} onError={e => { e.currentTarget.style.display="none"; e.currentTarget.nextSibling.style.display="flex"; }} />
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:"0.75rem", marginBottom:"1.5rem", flexWrap:"wrap" }}>
          ...
          <button onClick={copyLink}
            ...
            onMouseLeave={e => { if (copyLabel !== "Copied!") { e.currentTarget.style.background="rgba(255,255,255,0.06)"; e.currentTarget.style.color="#94a3b8"; }}}>
            <CopyIcon /> {copyLabel}
          </button>
          <div style={{ width:1, height:36, background:"rgba(255,255,255,0.08)" }} />
          <button onClick={shareOnTwitter} title="Share on X" style={{ ...iconBtnBase, color:"#1d9bf0", borderColor:"rgba(29,155,240,0.25)", background:"rgba(29,155,240,0.08)" }} onMouseEnter={e => { e.currentTarget.style.background="rgba(29,155,240,0.18)"; e.currentTarget.style.transform="translateY(-1px)"; }} onMouseLeave={e => { e.currentTarget.style.background="rgba(29,155,240,0.08)"; e.currentTarget.style.transform="translateY(0)"; }}><XIcon /></button>
          <button onClick={shareOnWhatsApp} title="Share on WhatsApp" style={{ ...iconBtnBase, color:"#25d366", borderColor:"rgba(37,211,102,0.25)", background:"rgba(37,211,102,0.08)" }} onMouseEnter={e => { e.currentTarget.style.background="rgba(37,211,102,0.18)"; e.currentTarget.style.transform="translateY(-1px)"; }} onMouseLeave={e => { e.currentTarget.style.background="rgba(37,211,102,0.08)"; e.currentTarget.style.transform="translateY(0)"; }}><WhatsAppIcon /></button>
          <button onClick={shareOnFacebook} title="Share on Facebook" style={{ ...iconBtnBase, color:"#1877f2", borderColor:"rgba(24,119,242,0.25)", background:"rgba(24,119,242,0.08)" }} onMouseEnter={e => { e.currentTarget.style.background="rgba(24,119,242,0.18)"; e.currentTarget.style.transform="translateY(-1px)"; }} onMouseLeave={e => { e.currentTarget.style.background="rgba(24,119,242,0.08)"; e.currentTarget.style.transform="translateY(0)"; }}><FacebookIcon /></button>
        </div>
        ...
        <div style={{ marginTop:"2.5rem", textAlign:"center" }}>
          <p style={{ fontSize:"0.74rem", color:"#334155", margin:0 }}>
            Made with 💜 by{" "}
            <a href="https://arniloy.framer.website" target="_blank" rel="noopener noreferrer" style={{ color:"#334155", textDecoration:"none" }} onMouseEnter={e=>e.currentTarget.style.color="#64748b"} onMouseLeave={e=>e.currentTarget.style.color="#334155"}>Arifin Rahman Niloy</a>
          </p>
        </div>
      </div>

      {/* Poster */}
      <div ref={posterRef} style={{ display:"none", position:"fixed", top:0, left:0, width:405, height:720, zIndex:-1, flexDirection:"column", background:"linear-gradient(160deg, #0a0010 0%, #0d0520 50%, #0a0010 100%)", padding:"36px 32px", fontFamily:"'DM Sans', sans-serif", overflow:"hidden", boxSizing:"border-box" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", marginBottom:22, position:"relative", zIndex:1 }}>
          <div style={{ position:"relative", marginBottom:16 }}>
            <svg width="140" height="140" viewBox="0 0 140 140">
              ...
              <circle cx="70" cy="70" r="58" fill="none" stroke={band.color} strokeWidth="9" strokeDasharray={`${2 * Math.PI * 58}`} strokeDashoffset={`${2 * Math.PI * 58 * (1 - pct / 100)}`} strokeLinecap="round" transform="rotate(-90 70 70)" style={{ filter:`drop-shadow(0 0 6px ${band.color}80)` }} />
            </svg>
            <div style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%,-50%)", textAlign:"center" }}>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", gap:9, position:"relative", zIndex:1 }}>
          ...
          {sectionScores.map(sec => (
            <div key={sec.id} style={{ display:"flex", alignItems:"center", gap:8 }}>
              ...
              <div style={{ flex:1, height:5, background:"rgba(255,255,255,0.07)", borderRadius:3, overflow:"hidden" }}><div style={{ height:"100%", width:`${sec.pct}%`, background:sec.color, borderRadius:3 }} /></div>
...
function DownloadIcon() { return <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>; }
function CopyIcon() { return <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>; }
function RetakeIcon() { return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10"/></svg>; }
function XIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.748l7.73-8.835L1.254 2.25H8.08l4.26 5.633 5.905-5.633zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>; }
