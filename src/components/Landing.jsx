...
export default function Landing({ onStart, onHistory }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080b14", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"2rem 1.25rem 1rem", fontFamily:"'Syne', sans-serif", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"0.5rem" }}>
          {showHistory && (
            <button onClick={onHistory}
              style={{ display:"flex", alignItems:"center", gap:"0.4rem", padding:"0.4rem 0.875rem", background:"rgba(99,102,241,0.1)", border:"1px solid rgba(99,102,241,0.25)", borderRadius:20, color:"#818cf8", fontSize:"0.78rem", fontFamily:"'DM Sans', sans-serif", cursor:"pointer", transition:"all 0.2s" }}
          ...
          <a href="https://github.com/arniloyofficial/opencore" target="_blank" rel="noopener noreferrer"
            style={{ display:"flex", alignItems:"center", padding:"0.4rem 0.7rem", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:20, color:"#94a3b8", textDecoration:"none", transition:"all 0.2s" }}
      ...
      <div style={{ opacity:visible?1:0, transform:visible?"translateY(0)":"translateY(30px)", transition:"all 0.9s ease 0.2s", maxWidth:640, paddingTop:"5rem", width:"100%" }}>
        ...
        <h1 style={{ fontSize:"clamp(1.9rem, 5.3vw, 3.7rem)", margin:"0 0 1.5rem", lineHeight:1.2, letterSpacing:"-0.03em", fontWeight:800, background:"linear-gradient(135deg, #fff 0%, #a5b4fc 60%, #818cf8 100%)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text", paddingBottom:"0.1em", overflowWrap:"break-word", wordBreak:"break-word" }}>
      ...
      <div style={{ opacity:visible?1:0, transition:"all 1s ease 0.5s", marginTop:"3.5rem", display:"flex", gap:"0.6rem", flexWrap:"wrap", justifyContent:"flex-start", maxWidth:640, width:"100%" }}>
        {sections.map(([iconId, label, color]) => (
          <div key={label} className="section-pill" style={{ "--pill-color": color }}>
      ...
      <style>{`
        ...
        .section-pill { background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 20px; padding: 0.35rem 0.85rem; font-size: 0.78rem; color: #64748b; font-family: 'DM Sans', sans-serif; display: flex; align-items: center; gap: 0.4rem; transition: all 0.2s ease; cursor: default; }
        .section-pill:hover { border-color: var(--pill-color); color: var(--pill-color); background: rgba(255,255,255,0.06); }
        .section-pill:hover svg { stroke: var(--pill-color); }
        @media (max-width: 480px) { .disclaimer-text { text-align: center !important; } }
      `}</style>
    </div>
  );
}